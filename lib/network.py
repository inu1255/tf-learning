...
class Network():
    ...
    def define_model(self):
        ...
        with tf.name_scope('test'):
            ...
            self.single_input = tf.placeholder(tf.float32, shape=single_shape, name='single_input')
            self.single_prediction = tf.nn.softmax(model(self.single_input, train=False), name='single_prediction')
    ...
    def run(self, train_samples, train_labels, test_samples, test_labels, *, train_data_iterator, iteration_steps, test_data_iterator):
        ...
        '''
        self.writer = tf.train.Write('./board', tf.get_default_graph())
    ...
    def predict(self, image):
        ...
        with tf.Session(graph=tf.get_default_graph()) as session:
            self.saver.restore(session, self.save_path)
            result = session.run(self.single_prediction, feed_dict={self.single_input: image})
